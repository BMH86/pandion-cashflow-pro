...
async waitForAuth() {
    ...
    /**
     * Save current project to Firebase
     */
    ...
    /**
     * Debounced save - prevents excessive Firebase writes
     */
    ...
    /**
     * Load current project data into UI
     */
     ...
     * @param {string} costType - Cost type (Hard/Soft/TI)
     * @param {string} distributionMethod - Distribution method
     * @param {Object} distributionParams - Distribution parameters
    ...
    /**
     * Show edit budget category modal
     * @param {number} id - Category ID
     */
    ...
    /**
     * Calculate projections for a category
    ...
    /**
     * Recalculate all baseline projections
     */
    recalculateBaseline() {
    ...
    /**
     * Update actual spend for a category and month
     * @param {number} categoryId - Category ID
     * @param {number} month - Month index
     * @param {number} amount - Actual amount
     */
    ...
    /**
     * Show actuals modal for a category
     * @param {number} categoryId - Category ID
     */
    showActualsModal(categoryId) {
        if (typeof enterMonthActuals === 'function') {
            enterMonthActuals(0);
        } else {
     ...
     * @param {string} baseScenarioId - Base scenario to copy from
     ...
     * @param {string} scenarioId - Scenario ID to switch to
     */
    ...
    /**
     * Setup tooltips for all elements
     */
    setupTooltips() {
...
class CalculationEngine {
     ...
     * @returns {Object} Month-to-amount mapping
     */
    ...
    /**
     * Calculate straight line distribution
     * @param {number} amount - Total amount
     * @param {Object} params - Parameters
     * @param {number} maxMonths - Maximum months
     * @returns {Object} Distribution
...
window.addEventListener('beforeunload', () => {
    if (app && app.chartInstances) {
        app.chartInstances.forEach(chart => {
            if (chart && !chart.isDisposed()) {
                chart.dispose();
            }
        });
    ...
    if (app && app.unsubscribeCallbacks) {
        app.unsubscribeCallbacks.forEach(unsubscribe => unsubscribe());
    }
});
