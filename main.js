...
// NEW: HTML escaping utility for XSS protection
...
// SAFETY: Maximum project horizon to prevent infinite loops
const MAX_PROJECT_HORIZON = 1200; // 100 years maximum
...
class CashflowApp {
    ...
    computeProjectHorizon() {
        ...
        (this.projectData.budgetCategories || []).forEach(category => {
            ...
            if (manualDistribution && typeof manualDistribution === 'object') {
                Object.keys(manualDistribution).forEach(monthKey => {
                    ...
                    // SAFETY: Enforce maximum horizon to prevent performance issues
                    if (!Number.isNaN(monthIndex) && monthIndex < MAX_PROJECT_HORIZON) {
        ...
        // SAFETY: Cap horizon at maximum to prevent performance issues
        const calculatedHorizon = horizon > 0 ? Math.max(horizon, defaultHorizon) : defaultHorizon;
        return Math.min(calculatedHorizon, MAX_PROJECT_HORIZON);
    }
    ...
    // ENHANCED: Create debounced save with proper cleanup tracking
    createDebouncedSave() {
        let timeout;
        return (...args) => {
            clearTimeout(timeout);
    ...
    debouncedSave = this.createDebouncedSave();
                    ...
                    <div class="modal-content">
                        ...
                        <form id="edit-budget-form" data-category-id="${id}">
                            <div class="form-group">
                                ...
                                <input type="text" name="code" value="${escapeHtml(category.code)}" required>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label>Amount</label>
                                <input type="number" name="amount" value="${escapeHtml(category.amount)}" step="0.01" required>
                            </div>
                            <div class="form-group">
    ...
    renderBudgetTable() {
        ...
        this.projectData.budgetCategories.forEach(category => {
            ...
            row.innerHTML = `
                <td data-tooltip="Category code: ${escapeHtml(category.code)}">${escapeHtml(category.code)}</td>
                <td data-tooltip="${escapeHtml(category.name)}">${escapeHtml(category.name)}</td>
                ...
                <td>
                    <span class="cost-type ${escapeHtml(category.costType.toLowerCase())}"
                          data-tooltip="${escapeHtml(category.costType)} costs">
                        ${escapeHtml(category.costType)}
                    </span>
                </td>
                <td data-tooltip="Distribution: ${escapeHtml(category.distributionMethod)}">
                    ${escapeHtml(this.formatDistributionMethod(category.distributionMethod))}
                </td>
                <td>
    ...
    cleanup() {
        console.log('CashflowApp: Cleaning up resources...');
        ...
        console.log('CashflowApp: Cleanup complete');
    }
...
class VisualizationEngine {
    ...
    renderCashflowChart(containerId, data, options = {}) {
        try {
            ...
            // FIXED: Remove old chart and its event listeners to prevent memory leaks
            if (this.app.chartInstances.has(containerId)) {
                const oldChart = this.app.chartInstances.get(containerId);
                if (oldChart._resizeHandler) {
                    window.removeEventListener('resize', oldChart._resizeHandler);
                }
                oldChart.dispose();
                this.app.chartInstances.delete(containerId);
            }
            ...
            // Store resize handler reference for later cleanup
            const resizeHandler = () => {
...
// CLEANUP
// ============================================================================
// Note: Main cleanup is now handled by CashflowApp.cleanup() method
// This is a backup handler in case cleanup() wasn't called

window.addEventListener('beforeunload', () => {
    if (window.app?.chartInstances) {
    ...
    if (window.app?.unsubscribeCallbacks) {
        window.app.unsubscribeCallbacks.forEach(unsubscribe => {
            try {
                unsubscribe();
            } catch (error) {
                console.error('Error unsubscribing:', error);
