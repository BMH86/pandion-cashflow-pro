...
6. Create a .env file (see .env.example) with your Firebase credentials
7. In Firestore Rules, use the rules at the bottom of this file
8. NEVER commit .env file to version control (add to .gitignore)

SECURITY NOTE:
While Firebase API keys are designed to be public and used in client-side code,
it's still a best practice to:
1. Keep them in environment variables (not committed to git)
...
5. Use domain restrictions in Firebase Console
*/

// SECURITY: Load configuration from environment variables or use defaults
// For production, these should come from your build process or hosting platform
const firebaseConfig = {
  ...
  apiKey: window.FIREBASE_CONFIG?.apiKey || "AIzaSyCviNwLVxsLSQjblNPIh1hl_JfCWlmzGyw",
  authDomain: window.FIREBASE_CONFIG?.authDomain || "pandion-cashflow-pro.firebaseapp.com",
  projectId: window.FIREBASE_CONFIG?.projectId || "pandion-cashflow-pro",
  storageBucket: window.FIREBASE_CONFIG?.storageBucket || "pandion-cashflow-pro.firebasestorage.app",
  messagingSenderId: window.FIREBASE_CONFIG?.messagingSenderId || "1041555626841",
  appId: window.FIREBASE_CONFIG?.appId || "1:1041555626841:web:41bc36ae1434c40165ca48",
  measurementId: window.FIREBASE_CONFIG?.measurementId || "G-BWVHK93P9Y"
};
...
if (window.FIREBASE_CONFIG) {
  ...
  console.warn('⚠️ Firebase config using hardcoded defaults. For production, use environment variables!');
}
